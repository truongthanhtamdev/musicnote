...
export default function TeacherAttendanceHistoryRow({
  ...
  return (
    <tr className="hover:bg-ivory-50">
      ...
      <td className="px-2.5 sm:px-4 py-3 tabular text-ink-500 hidden sm:table-cell">{row.check_in_time || "–"}</td>
      <td className="px-2.5 sm:px-4 py-3 text-ink-600 max-w-[220px] hidden sm:table-cell">
        <span className="block truncate" title={row.lesson_content || undefined}>
          {row.lesson_content || "–"}
        </span>
      </td>
      <td className="px-2.5 sm:px-4 py-3 text-ink-500 max-w-[160px] hidden sm:table-cell">
