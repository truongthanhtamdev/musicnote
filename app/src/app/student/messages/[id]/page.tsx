...
export default async function StudentThreadPage({
  ...
  if (!cls || !canUseClassChat(cls, session.userId, "student")) notFound();

  return (
    <div className="space-y-5">
      <PageHeader
        title={cls.teacher_name || "Chưa xếp giáo viên"}
        subtitle={`${cls.subject} · ${formatClassSchedule(cls)}`}
        action={
          <div className="flex items-center gap-2">
            <JoinClassLink url={cls.meeting_url} />
            <Link href="/student/messages" className="text-sm font-semibold text-wood-600">
              ← Tất cả tin nhắn
            </Link>
          </div>
        }
